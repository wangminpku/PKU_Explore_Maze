...
//                            eItem.to.getIndex());
    ...
    private void drawLabels(Graphics g){
        for (int row = 0; row < boardy; row++) {
            for (int col = 0; col < boardx; col++) {
                ...
                g.drawString("(" + (col) + "," + (row + (col + 1) / 2) + ")", locx + 62, locy + 88);
    ...
    private void drawMapLabels(Graphics g){
    		ImageCache ic = ImageCache.getInstance();
        ...
        for (int row = 0; row < boardy; row++) {
            for (int col = 0; col < boardx; col++) {
                ...
                if (background != null) {
                    //g.drawImage(background, locx, locy, this);
                    g.drawString(v.getData().getTerrain().name().substring(1), locx+40, locy+40);
                }
                else {
                		g.drawString("Default", locx+40, locy+40);
                }
            }
        }
    }
